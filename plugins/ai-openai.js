...
const configuration = new Configuration({ organization: `org-wlQ2hrPSZIepQa7QcTaG7Lh5`, apiKey: `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` }); 
const openai = new OpenAIApi(configuration);

let handler = async (m, { conn, usedPrefix, command, text }) => {
  try {
    if (!text) throw new Error(`Contoh:\n${usedPrefix}${command} Apa Itu Cinta?`);
